...
const   InsumosCortadores = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:8000/api/Insumos", {withCredentials: true})
            .then(res =>  {
                setInsumos(res.data)
    ...
    const borrarInsumo = id => {
        axios.delete("http://localhost:8000/api/insumos/"+id, {withCredentials: true})
            .then(res => {
                //Actualizar ls lita de autores por medio de filter
                let nuevaLista = insumos.filter(insumo => insumo._id !== id);
                setInsumos(nuevaLista);
            })
    }
    ...
    return (
            ...
            <nav className='bg-[#ffe5b1] flex justify-between lg:justify-start items-center sticky top-0 z-40 border-b-2 border-[#44413a] '>
                ...
                <div className='links lg:block hidden w-100 md:w-100 ml-[50px]'>
                    <ul className='menu flex items-center justify-start gap-10 '>
                        ...
                        <li><a href='/CursosAdmi' className='link inline-block p-4 rounded-t-lg hover:text-gray-600 hover:bg-[#ffc757] dark:hover:bg-gray-800 dark:hover:text-gray-300'>Cursos</a></li>
        ...
        <div class="bg-[url('../public/img/fondo.png')]">
                ...
                <div class="flex  flex-row-reverse space-x-4 pt-4 pr-5">
                        <Link to="/NuevoInsumo" class="py-1 px-4  bg-[#3fff3b] text-white font-bold border-transparent border-2 border-black text-1 rounded-3xl hover:border-black hover:bg-[#80e67f] hover:text-black transition duration-200'" type="submit">
                                        Agregar Insumo
                            </Link>
                </div>
                    <div className="flex flex-row gap-6 mb-6 pt-5 md:grid-cols-2 font-bold">
                        <div className="clear-left ml-20 w-[350px] pt-10">
                            <div className="bg-[#ffc757] pb-10 border-4 border-black">
                                  <h2 class="mb-2 text-3xl font-semibold text-gray-900 dark:text-white pt-14 ">Insumos</h2>
                            <ul class="space-y-1 max-w-md list-inside text-black dark:text-gray-400 ">
                                <li>
                                    <a href='/InsumoMoldesSilicona' class="font-medium  dark:text-blue-500 hover:underline text-1xl">Moldes de silicona</a>
                                    
                                </li>
                                <li>
                                <a href='/InsumosCortadores' class="font-medium  dark:text-blue-500 hover:underline text-1xl">
                                     Cortadores
                                </a>
                                   
                                </li>
                                <li>
                                <a href='/InsumoTonalizadores' class="font-medium  dark:text-blue-500 hover:underline text-1xl">Tonalizadores</a>
                        ...
                        <div className="clear-left ml-20 w-[350px] pb-10 mb-2 text-3xl font-semibold text-gray-900 dark:text-white">
                                <h3 className="text-5xl ">Cortadores</h3>
                            </div>

                            {insumos.map((insumo, index) => 
                            {
                                
                            if(insumo.categoriaInsumos == "Cortadores") {
                             ...
                             return   (
                                ...
                                <div class="flex-none w-48 mb-10 relative  z-30 before:absolute before:top-1 before:left-1 before:w-full before:h-full before:bg-yellow-500 rounded-lg">
                                    <img src={insumo.imagen } alt="" class="absolute z-10 inset-0 w-full h-full object-cover rounded-lg" loading="lazy" />
                                </div>
                                <form class="flex-auto pl-6">
                                 
                                    <div class="relative flex flex-wrap items-baseline pb-6 before:bg-[#36332a] before:absolute before:-top-6 before:bottom-0 before:-left-60 before:-right-6 w-[200px]">
                                    ...
                                    <div class="flex space-x-4 pt-6">
                                        <Link to={`/ActualizarInsumoCortadores/${insumo._id}`} class="py-1 px-4  bg-[#ffb41e] text-white font-bold border-transparent border-2 border-black text-1 rounded-3xl hover:border-black hover:bg-[#ffc85a] hover:text-black transition duration-200'" type="submit">
                                        Editar
                                        </Link>
                                        <button  onClick={() => borrarInsumo(insumo._id)} class="py-1 px-4  bg-[#cc2c2c] text-white font-bold border-transparent border-2 border-black text-1 rounded-3xl hover:border-black hover:bg-[#fc7676] hover:text-black transition duration-200'" type="submit">
                                        Borrar
                                        </button>
                                    </div>
...
export default InsumosCortadores;
